...
const toggleLikeAPI = async(productName:string) : Promise<String> => {
    ...
    const { mutate : toggleLike } = useMutation({
        ...
        onMutate : async (targetName) => {
            ...
            queryClient.setQueryData(['products'], (old: ProductInfo[] | undefined ) => {
                if(!old) return [];
                return old.map(item => item.productName === targetName ? { ...item, isLiked : !item.isLiked } : item)
            })
        ...
        onError : (err, targetName, context) => {
            alert(`앗! ${targetName} 찜하기에 실패했습니다... ${err.message}`);
    ...
    return (
        ...
        <div className="p-5 border border-gray-200 rounded-xl shadow-sm flex flex-col gap-3">
            <div className="text-lg font-bold">
                ...
                {/* 💡 찜하기 버튼 추가! */}
                <button 
                    onClick={() => toggleLike(productName)}
